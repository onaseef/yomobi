...
dbxGroup.prototype.dbxifyBox = function (box) {
  ...
  //the dbxid token is based on the boxes length counter
	...
	if(dbx.hasClass(box, 'dbx\-box') && !dbx.hasClass(box, 'dbx\-(dummy|clone)'))
	{
		//if box-ID based dynamic groups are enabled
		if(dbx.useid)
		{
			//if it has a valid ID, use that for dbxid
			if(/^[a-z][a-z0-9]*$/i.test(box.id) && !/^(length|dummy)$/.test(box.id))
			{
				dbxid = box.id;
			}
			...
			{
				throw('Error from dbxGroup:\n"' + box.id + '" is an invalid box ID');
				return;
			}
		}

		//add to dictionary of docking boxes and increase the length counter
		this.boxes[dbxid] = box;
		this.boxes.length++;
		...
		if(typeof box.hashandlers == 'undefined')
		{
			//bind mousedown handler
			dbx.addEvent(box, 'mousedown', function(e)
			{
		...
		if(typeof box.processed != 'undefined') { return; }
	...
	if(dbx.hasClass(box, 'dbx\-handle'))
	{
		...
		if(this.toggles)
		{
			...
			this.buttons[dbxid] = this.addToggleBehavior(box);
		}
		...
		else if(typeof box.hashandlers == 'undefined')
		{
		...
		var oldtitle = box.getAttribute('title');
		if(oldtitle) { box.setAttribute('oldtitle', oldtitle); }
		...
		if(!dbx.hasClass(parentbox, 'dbx\-nograb'))
		{
			...
			box.className += ' dbx-handle-cursor';
			...
			box.setAttribute('title', 
				dbx.empty(box.getAttribute('title'))
					? this.vocab.move 
					: this.vocab.syntax.replace(/%mytitle[%]?/, box.title)
	...
	if(dbx.msie && dbx.hasClass(box, 'dbx\-(content|box)'))
	{
		box.runtimeStyle.zoom = '1.0';
	}
}
